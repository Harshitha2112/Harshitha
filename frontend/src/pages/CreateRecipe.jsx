...
const CreateRecipe = () => {
  ...
  const [form, setForm] = useState({
    ...
    cookingTime: "", servings: "", difficulty: "Easy",
    tags: "",
  });
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.75rem" }}>
        ...
        <div className="glass-card" style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1.1rem" }}>
          ...
          <div style={inputStyle}>
            ...
            <input name="title" required className="input-field" placeholder="e.g. Creamy Butter Chicken" value={form.title} onChange={handleChange} />
          </div>

          <div style={inputStyle}>
            <label style={labelStyle}>Description *</label>
            <textarea name="description" required rows={3} className="input-field" placeholder="Describe your recipe in a few sentences..."
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1rem" }}>
            ...
            <div style={inputStyle}>
              ...
              <input name="cookingTime" type="number" min="1" required className="input-field" placeholder="30" value={form.cookingTime} onChange={handleChange} />
            </div>
            <div style={inputStyle}>
              <label style={labelStyle}>Servings *</label>
              <input name="servings" type="number" min="1" required className="input-field" placeholder="4" value={form.servings} onChange={handleChange} />
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
            ...
            <div style={inputStyle}>
              ...
              <input name="tags" className="input-field" placeholder="italian, quick, healthy" value={form.tags} onChange={handleChange} />
            </div>
          </div>
        </div>
        ...
        <div className="glass-card" style={{ padding: "1.5rem" }}>
          ...
          <label htmlFor="image-upload" style={{ cursor: "pointer" }}>
            <div style={{
              ...
              {imagePreview ? (
                <img src={imagePreview} alt="preview" style={{ maxHeight: "240px", borderRadius: "0.5rem", objectFit: "cover" }} />
              ) : (
                <>
                  <Upload size={32} color="var(--accent)" style={{ margin: "0 auto 0.75rem" }} />
                  <p style={{ color: "var(--text-secondary)", fontSize: "0.875rem" }}>Click to upload a photo</p>
                  <p style={{ color: "var(--text-secondary)", fontSize: "0.75rem" }}>JPG, PNG, WEBP • Max 5MB</p>
                </>
              )}
            </div>
          </label>
        ...
        <div className="glass-card" style={{ padding: "1.5rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.65rem" }}>
            {ingredients.map((ing, i) => (
              <div key={i} style={{ display: "flex", gap: "0.65rem", alignItems: "center" }}>
                <input className="input-field" placeholder="Quantity (e.g. 2 cups)" value={ing.quantity}
                  onChange={e => handleIngredient(i, "quantity", e.target.value)} style={{ width: "160px", flexShrink: 0 }} />
                <input className="input-field" placeholder="Ingredient name" value={ing.name}
                  onChange={e => handleIngredient(i, "name", e.target.value)} style={{ flex: 1 }} />
                {ingredients.length > 1 && (
                  <button type="button" onClick={() => removeIngredient(i)} style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: "0.5rem", padding: "0.5rem", cursor: "pointer", color: "#ef4444", flexShrink: 0 }}>
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
          ...
          <button type="button" onClick={addIngredient} className="btn-secondary" style={{ marginTop: "0.75rem", padding: "0.5rem 1rem", fontSize: "0.8rem" }}>
        ...
        <div className="glass-card" style={{ padding: "1.5rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {steps.map((step, i) => (
              <div key={i} style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
                <span style={{ background: "var(--accent)", color: "#fff", borderRadius: "50%", width: "30px", height: "30px", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "0.8rem", flexShrink: 0, marginTop: "8px" }}>
                  {step.stepNumber}
                </span>
                <textarea className="input-field" rows={2} placeholder={`Describe step ${step.stepNumber}...`}
                  value={step.instruction} onChange={e => handleStep(i, e.target.value)} style={{ flex: 1, resize: "vertical" }} />
                {steps.length > 1 && (
                  <button type="button" onClick={() => removeStep(i)} style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: "0.5rem", padding: "0.5rem", cursor: "pointer", color: "#ef4444", flexShrink: 0, marginTop: "8px" }}>
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
          ...
          <button type="button" onClick={addStep} className="btn-secondary" style={{ marginTop: "0.75rem", padding: "0.5rem 1rem", fontSize: "0.8rem" }}>
